...
práticas. Para que essa experiência se traduza em melhoria pedagógica contínua,
é necessário dispor de instrumentos com visibilidade e rastreabilidade
do caminho que leva ao produto final — isto é, o processo — e não apenas do
resultado entregue em marcos específicos.
...
altamente dinâmico, o que torna a observação sujeita à subjetividade e à
intermitência quando apoiada apenas em instrumentos pontuais (provas,
apresentações e rubricas de produto). Soma-se a isso a fragmentação dos dados
...
integrada que subsidie decisões pedagógicas informadas e a orquestração do processo
...
ecossistema PBL e apoiar a decisão docente, sem mensurar desempenho discente.
...
\begin{itemize}
  ...
  \item Avaliar a arquitetura e o pipeline por métodos de avaliação
        ...
        throughput, perda de eventos, integridade e confiabilidade), sem
        qualquer mensuração de desempenho discente.
...
sem mensurar desempenho discente, manifestando-se por:
...
desenvolvidos, os processos adotados e as evidências de atividade, sem
mensuração de desempenho discente \cite{hmelo2004}.
...
\textbf{Fase 3 - Exemplo Aplicado (Maio-Julho 2026)}: Implementação prática do modelo proposto em contexto real de PBL, incluindo seleção do ambiente educacional, especificação de requisitos específicos do contexto, escolha criteriosa de tecnologias adequadas, desenvolvimento do protótipo funcional e coleta de dados empíricos do repositório Git para demonstrar a captura de eventos.

\textbf{Fase 4 - Análise de Resultados (Julho-Setembro 2026)}: Processamento e análise dos dados coletados durante o exemplo aplicado (captura de eventos Git), avaliação do comportamento arquitetural do pipeline e documentação dos resultados obtidos.
...
modelo conceitual validado, os resultados do exemplo aplicado, e as
contribuições científicas da pesquisa para as áreas de educação em engenharia e
tecnologias educacionais emergentes.

\section{MODELO PROPOSTO}

Este capítulo apresenta a arquitetura para captura, integração e organização de eventos que alimentam um gêmeo digital no contexto de PBL em engenharia de software. O modelo integra três visões arquiteturais (estrutural, comportamental e de processo) conforme ISO/IEC/IEEE 42010:2022, proporcionando visibilidade e rastreabilidade do processo, alinhadas às perspectivas institucionais e técnicas.
...
% [Figura removida: Diagrama macro simplificado]
...
\begin{figure}[H]
    \centering
    % [Figura removida: diagrama1_modelo_macro]
...
\begin{figure}[H]
    \centering
    % [Figura removida: diagrama2_estrutura_operacional]
\end{figure}
...
\begin{figure}[H]
    \centering
    % [Figura removida: diagrama3_produtos_sprint]
\end{figure}
...
\begin{figure}[H]
    \centering
    % [Figura removida: diagrama4_artefatos_tecnicos]
\end{figure}
...
% [Removido: descrição institucional de avaliação por pontos]
...
\begin{figure}[H]
    \centering
    % [Figura removida: diagrama5_metricas_indicadores]
\end{figure}
...
\begin{figure}[H]
    \centering
    % [Figura removida: diagrama6_modelo_integrado]
\end{figure}
...
% [Removido: eixos institucionais de avaliação]
...
\section{EXEMPLO APLICADO: CAPTURA DE EVENTOS EM REPOSITÓRIO GIT}
...
\begin{figure}[H]
  \centering
  % [Figura removida: afonsystem_class]
\end{figure}
...
\begin{figure}[H]
  \centering
  % [Figura removida: afonsystem_sequence]
\end{figure}
...
\begin{figure}[H]
  \centering
  % [Figura removida: afonsystem_components]
\end{figure}
