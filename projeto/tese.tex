...
\textbf{\large Arquitetura de integração de uma plataforma PBL com gêmeo digital, com IA}
\vskip 4.5cm
\end{center}
...
intermitência quando apoiada apenas em instrumentos pontuais (provas, apresentações e rubricas de produto). Soma-se a isso a fragmentação dos dados relevantes em múltiplas plataformas (repositórios de código, gestão de tarefas, mensageria, ambientes de aula), dificultando a construção de uma visão integrada que subsidie decisões pedagógicas informadas e a orquestração do processo \cite{romero2010, romero2020}. Em resumo, faltam meios sistemáticos para observar o processo em curso e para transformar essa observação em evidências interpretáveis por diferentes públicos.

Nesse cenário, os Gêmeos Digitais (Digital Twins) despontam como tecnologia
capaz de aproximar a observação do PBL daquilo que se pretende acompanhar: o
...
com seus referentes, permitindo acompanhamento e análises sob demanda.
Transpostos para o contexto educacional, esses recursos podem ampliar a
visibilidade sobre o que acontece, quando e como acontece — especialmente
...
abre-se a oportunidade de explorar gêmeos digitais como instrumentos de
observabilidade capazes de subsidiar a tomada de decisão de docentes e tutores.
...
de atividades e marcos operacionais. Ao integrar essas visões,
...
concepção e a verificação arquitetural do artefato proposto.

Apesar de avanços relatados, persiste uma lacuna no emprego de gêmeos digitais
como mecanismo de captura e integração de eventos em projetos PBL na
engenharia de software. As iniciativas mais frequentes concentram-se na
construção de protótipos físicos e virtuais sem enfrentar de forma abrangente
duas questões centrais: (i) como observar — de modo ético e objetivo — os
processos em andamento ao longo do tempo; e (ii) como transformar esses dados
em insumos úteis para coordenação e tutoria. Como consequência, decisões
críticas podem ocorrer tardiamente, padrões de colaboração passam
despercebidos e o alinhamento entre objetivos, processos e produtos fica pouco
transparente. Diante desse quadro, torna-se premente conceber arquiteturas de
captura e integração de eventos que satisfaçam requisitos de qualidade
(efetividade, eficiência, sustentabilidade), ao mesmo tempo em que sejam
sensíveis à complexidade própria de programas de engenharia.

Neste trabalho, a necessidade de pesquisa é, portanto, dupla: de um lado,
propor uma arquitetura de gêmeo digital focada em capturar, integrar e
organizar eventos (artefatos, atividades e interações) e, de outro, demonstrar
que tal integração pode subsidiar decisões docentes e a orquestração do PBL.
Para isso, delineiam-se requisitos que orientam a solução: (a) confiabilidade e
integridade de dados; (b) temporalidade adequada às rotinas pedagógicas; (c)
interpretabilidade para diferentes públicos; (d) interoperabilidade com
ferramentas correntes de desenvolvimento e colaboração; e (e) observância a
princípios éticos e de privacidade.

\subsection{Objetivo Geral}

À luz do problema delineado, o objetivo geral desta pesquisa é projetar e
especificar uma arquitetura de captura, integração e organização de eventos de
ensino e aprendizagem que integre as visões arquiteturais estrutural,
comportamental e de processo, de forma a alimentar um gêmeo digital do
ecossistema PBL e subsidiar a tomada de decisão do corpo docente.
...
Se uma arquitetura de captura, integração e organização de eventos for
projetada e integrada às visões arquiteturais estrutural, comportamental e de
processo, então ela deverá consolidar, com rastreabilidade e confiabilidade,
dados de diferentes fontes do ecossistema PBL para apoiar a decisão docente,
manifestando-se por:

\begin{itemize}
  \item \textbf{Cobertura de fontes e completude de eventos}: integração de repositórios Git, gestão de tarefas e fontes institucionais com perdas toleráveis e consistência temporal;
  \item \textbf{Rastreabilidade fim a fim}: encadeamento estável de chaves (pessoas‑equipes‑sprints‑artefatos‑eventos) do ponto de coleta à materialização no gêmeo digital;
  \item \textbf{Latência e throughput compatíveis}: atualização dos dados em janelas temporais adequadas às rotinas do PBL;
  \item \textbf{Resiliência a falhas}: tolerância a eventos fora de ordem, reprocessamento e reconciliação;
  \item \textbf{Interoperabilidade/portabilidade}: conectores e contratos de dados que facilitem adoção em diferentes ecossistemas.
\end{itemize}
...
\textbf{H1 — Cobertura e Consistência}: O pipeline captura e integra, com perdas
...
\textbf{H2 — Latência e Throughput}: A ingestão e o processamento atendem a
limiares temporais definidos para atualização e análise sob demanda.

\textbf{H3 — Confiabilidade e Resiliência}: O sistema lida com eventos
...
\textbf{H4 — Interoperabilidade e Portabilidade}: A arquitetura, baseada em
contratos e conectores, integra-se a diferentes ecossistemas PBL, alinhada a
viewpoints RM‑ODP (ISO 10746) e à ISO 42010.

\textbf{H5 — Observabilidade e Rastreabilidade}: A solução oferece trilhas de
...
escopo e ética criam condições de validade para todas as hipóteses (H1–H5);
(ii) o modelo conceitual e os metamodelos dão base a H1–H4; (iii) a
padronização de eventos e contratos de dados sustenta H1–H3 e contribui para
H4; (iv) a arquitetura e o pipeline viabilizam evidências integradas para
H1–H4; (v) o protótipo e os painéis tornam observáveis H1–H4 no contexto real;
(vi) o exemplo aplicado fornece dados para aceitar/refutar H1–H5; e (vii) as
diretrizes e análise de limitações informam H5 (transferibilidade/escala).

\subsubsection{Critérios de Validação}

Serão utilizados critérios arquiteturais e operacionais, incluindo:

\textbf{Critérios Arquiteturais e Operacionais} (alinhamento a hipóteses):
\begin{itemize}
  \item \textbf{Cobertura de fontes} (H1): percentual de projetos/artefatos/eventos coletados por conector, taxa de perda e de duplicatas;
  \item \textbf{Integridade e consistência} (H1, H5): validação de esquemas, ordenação temporal, unicidade de chaves e reconciliação de eventos fora de ordem;
  \item \textbf{Latência e throughput} (H2): tempo mediano de ingestão/atualização e capacidade sustentada do pipeline;
  \item \textbf{Confiabilidade} (H3): taxa de erros de coleta/processamento, disponibilidade, MTTR e sucesso de reprocessamentos;
  \item \textbf{Interoperabilidade} (H4): esforço de integração medido por conectores/contratos e aderência a viewpoints RM‑ODP (ISO 10746) e ISO 42010;
  \item \textbf{Rastreabilidade e observabilidade} (H5): existência de trilhas de auditoria fim a fim e recuperabilidade de encadeamentos.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{OE3 — Métricas e indicadores}: Entregáveis — catálogo de métricas e
        ...
        de eventos e integrações do gêmeo digital).
  ...
  \item \textbf{OE6 — Demonstração/exemplo aplicado}: Entregáveis — protocolo e relato do
        exemplo aplicado em disciplina PBL; Seções — Metodologia (Fase 3: Exemplo Aplicado).
...
internacional. A aplicação sistemática de gêmeos digitais para observabilidade
...
A aplicação de gêmeos digitais no contexto educacional está em expansão. Silveira e Martins
...
Esta seção explicita, com base na revisão sistemática e no levantamento bibliométrico conduzidos, o lugar desta tese no estado da arte sobre avaliação em Aprendizagem Baseada em Projetos (ABP) na Engenharia de Software. Apresentamos o escopo e a questão de pesquisa (PICO), a estratégia de busca e seleção (protocolo de Kitchenham \cite{kitchenham2004}), o funil de triagem, as visões analíticas adotadas (inspiradas no RM‑ODP \cite{iso10746}) e as lacunas sintetizadas que orientam a proposta arquitetural desta tese.

\subsection{Questão e Escopo (PICO)}

A formulação da questão de pesquisa seguiu o framework \textbf{PICO} (População, Intervenção, Comparação, Resultados), recomendado para revisões sistemáticas em Engenharia de Software \cite{kitchenham2004} e consolidado na literatura metodológica \cite{santos2007pico}. Essa escolha assegura uma ancoragem explícita entre a pergunta orientadora, os termos de busca e os critérios de seleção, reduzindo vieses e reforçando a rastreabilidade do protocolo.

No contexto desta tese, a População considera o ensino superior — com ênfase em Engenharia de Software — em ambientes autênticos de Aprendizagem Baseada em Projetos. A Intervenção abrange métodos, instrumentos, arquiteturas e tecnologias de apoio à avaliação em ABP, com interesse particular em monitoramento processual e feedback formativo. A Comparação não foi definida como requisito obrigatório, sendo utilizada apenas quando estudos alternativos eram informativos para o tema. Os Resultados de interesse concentram-se na objetividade (métricas e validação), na escalabilidade (automação e padronização) e na avaliação processual (capacidade de acompanhar e retroalimentar o processo em curso).

O PICO orientou de ponta a ponta o protocolo: guiou a decomposição da pergunta em facetas para a construção das strings de busca; fundamentou os critérios de inclusão e exclusão — ao exigir alinhamento explícito aos elementos P, I e O —; e estruturou o formulário de extração, de modo que intervenções e resultados fossem registrados de forma consistente. O mapeamento entre PICO e questões de pesquisa (RQs) foi documentado no protocolo e serviu de base para a síntese descritiva dos achados.

Em consonância, o protocolo de revisão adotou as três fases de Kitchenham \cite{kitchenham2004}. No planejamento, justificaram-se escopo e necessidade da revisão, formalizou-se a questão (via PICO), definiram-se as visões analíticas (RM‑ODP), os critérios e o plano de análise. Na condução, realizaram-se as buscas na Web of Science, com documentação das strings e filtros; a triagem ocorreu no Rayyan, seguida de avaliação de qualidade e extração padronizada. No relato, consolidou-se a síntese descritiva, registrou-se com transparência o funil (882 \,$\to$\, 301 \,$\to$\, 20) e organizaram-se os artefatos para auditabilidade e reuso. Esse encadeamento confere clareza metodológica ao posicionamento e sustenta os requisitos do modelo proposto.

\subsection{Estratégia Bibliométrica e Bases}

Adotou‑se a \textbf{Web of Science} como única base de dados para a coleta principal, por combinar curadoria rigorosa (peer review), amplitude multidisciplinar e facilidades de exportação padronizada (RIS/DOI), em conformidade com Kitchenham \cite{kitchenham2004}. Idiomas não foram restritos na busca; na prática, os resultados recuperados foram predominantemente em inglês. As strings foram elaboradas de forma incremental e validadas por buscas piloto, decompondo a questão em facetas (escopo ABP, avaliação e contexto educacional) e adicionando termos específicos conforme as visões analíticas adotadas.

Para garantir cobertura e reduzir vieses na coleta, estruturamos a busca segundo as cinco visões do \textbf{Reference Model of Open Distributed Processing (RM‑ODP, ISO/IEC 10746)} \cite{iso10746}. Essas visões funcionam aqui como lentes complementares para mapear o problema de avaliação em ABP: a \emph{visão empresarial} enquadra propósitos, políticas e atores institucionais; a \emph{visão de informação} foca instrumentos e fluxos informacionais (rubricas, dashboards, analytics); a \emph{visão computacional} privilegia objetividade, padronização e validação; a \emph{visão de engenharia} orienta mecanismos de monitoramento processual por eventos/telemetria; e a \emph{visão de tecnologia} trata da instrumentação de competências transversais e de plataformas concretas. O uso coordenado dessas visões aumentou a cobertura temática e preservou a rastreabilidade entre requisitos, dados e soluções analisadas. As strings foram, portanto, especializadas por visão, combinando termos‑base de ABP e avaliação com termos específicos e, quando necessário, filtros de exclusão.

\subsection{Funil de Seleção e Critérios}

A coleta multi‑camadas retornou 882 registros totais. Após retenção de DOIs e deduplicação inter‑camadas por DOI, consolidou‑se um conjunto com \textbf{301 artigos}. A triagem seguiu o protocolo de Kitchenham \cite{kitchenham2004}: títulos e resumos foram avaliados em ambiente de triagem (Rayyan), aplicando critérios de inclusão/exclusão definidos no protocolo; os estudos potencialmente elegíveis passaram à leitura completa, extração padronizada e síntese descritiva. O conjunto final incluiu \textbf{20 artigos}.

\paragraph{Ferramenta de triagem (Rayyan).}
Para operacionalizar a etapa de seleção utilizamos o \textbf{Rayyan} (\url{https://www.rayyan.ai}), uma plataforma web voltada a revisões sistemáticas e mapeamentos, que oferece organização de referências, triagem colaborativa (com possibilidade de cegamento), marcação estruturada de critérios, busca e filtros avançados e registro de razões de exclusão. Neste estudo, os registros consolidados (formato RIS, com DOI) foram importados para o Rayyan e triados segundo os \textbf{critérios de inclusão e exclusão} definidos no protocolo em conformidade com Kitchenham \cite{kitchenham2004}. Cada decisão foi justificada por rótulos padronizados, e os estudos classificados como “potencialmente incluídos” seguiram para leitura completa e extração padronizada, garantindo transparência e reprodutibilidade do funil.
...
Para assegurar consistência e relevância do corpus, os critérios de elegibilidade foram definidos a priori e aplicados de forma sistemática. Foram considerados elegíveis estudos que tratassem de avaliação em contextos de Aprendizagem Baseada em Projetos no ensino superior, preferencialmente em cursos de Engenharia/Computação; que descrevessem métodos, instrumentos, arquiteturas ou tecnologias com potencial de apoiar a avaliação (do processo e/ou do produto); e que apresentassem desenho empírico ou relato técnico suficientemente detalhado para permitir extração padronizada de evidências. Exigiu‑se ainda publicação com revisão por pares e identificação por DOI, sem restrição prévia de idioma (na prática, predominou o inglês).

Foram excluídos trabalhos que não abordassem ABP ou que não tratassem de avaliação; estudos restritos à educação básica (K–12) ou a contextos sem transponibilidade para o ensino superior em Engenharia/Computação; peças opinativas e editoriais sem base empírica; duplicatas entre camadas de busca após normalização por DOI; e registros que não disponibilizassem metadados mínimos para triagem e extração reprodutível.

Do ponto de vista da cobertura temática, o consolidado (n=301) revelou assimetrias características do campo. A perspectiva de Engenharia — crucial para avaliação processual em tempo real — é praticamente ausente. As visões Empresarial e de Informação aparecem com maior frequência, mas dispersas entre instrumentos e justificativas que raramente convergem para modelos integrados. Na visão Computacional, observa‑se baixa padronização de métricas e escassez de estudos com validação explícita (confiabilidade/validade). Já a visão de Tecnologia tende a privilegiar resultados finais e percepções, com pouca instrumentação longitudinal de competências transversais. Esse panorama corrobora a oportunidade e orienta as decisões arquiteturais assumidas nesta tese.

\subsection{Síntese das Lacunas e Direcionadores}

Da síntese emergem lacunas que direcionam a presente tese: (i) \textbf{integração informacional} entre dados técnicos, colaborativos e formativos (superando instrumentos isolados); (ii) \textbf{objetividade e escalabilidade} por meio de métricas operacionais com protocolos de validação (confiabilidade interavaliadores, reprodutibilidade); (iii) \textbf{avaliação processual} com \textbf{arquitetura orientada a eventos} para monitoramento contínuo/feedback; (iv) \textbf{instrumentação de competências transversais} com sinais observáveis no tempo. Essas lacunas mapeiam diretamente para as visões 2–4 (com 5 como dimensão de dados) e fundamentam os requisitos do modelo proposto.

\subsection{Posicionamento desta Tese}

Esta tese propõe uma \textbf{abordagem arquitetural integrada} — formalizada como gêmeo digital educacional — que: (a) define um \textbf{modelo de informação} para integrar evidências de processo e produto; (b) implementa um \textbf{pipeline computacional} com métricas objetivas e validação; e (c) adota um \textbf{backbone orientado a eventos} para avaliação processual e feedback formativo em ABP. O desenho ancora‑se em visões arquiteturais (ISO/IEC/IEEE 42010), mantendo rastreabilidade entre evidências, critérios e decisões pedagógicas.

\subsection{Protocolo e Reprodutibilidade}

O protocolo (Kitchenham \cite{kitchenham2004}) foi documentado nas fases de \emph{planejamento, condução e relato}, incluindo strings por visão, critérios, planilhas de extração e artefatos de exportação (RIS/DOI). A transparência do funil (882 \textrightarrow{} 301 \textrightarrow{} 20) e a descrição das visões asseguram auditabilidade e reuso. A seção de Metodologia detalha os procedimentos complementares de validação utilizados nesta tese.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|}
    \hline
    \textbf{Atividade}      & \textbf{Ago} & \textbf{Set} & \textbf{Out} & \textbf{Nov} & \textbf{Dez} & \textbf{Jan} & \textbf{Fev} & \textbf{Mar} & \textbf{Abr} & \textbf{Mai} & \textbf{Jun} & \textbf{Jul} & \textbf{Ago} & \textbf{Set} & \textbf{Out} \\
    \hline
...
Exemplos Aplicados        &              &              &              &              &              &              & X            & X            & X            & X            & X            & X            &              &              &              \\
    ...
    \hline
    \textbf{Defesa de Tese}   &              &              &              &              &              &              &              &              &              &              &              &              &              &              & \textbf{X} \\
    \hline
  \end{tabular}%
  }
\end{table}

\textbf{Fase 1 - Pesquisa Bibliográfica (Agosto-Novembro 2025)}: Revisão sistemática da literatura sobre gêmeos digitais aplicados à educação, metodologias PBL em engenharia de software, e frameworks de avaliação educacional. Esta fase inclui refinamentos até novembro.

\textbf{Fase 2 - Proposição do Modelo (Setembro/2025–Março/2026)}: Desenvolvimento do modelo conceitual de gêmeo digital, incluindo a especificação das três visões arquiteturais (estrutural, comportamental e de processo), a definição de métricas e indicadores de avaliação e a explicitação de requisitos funcionais e não funcionais, com iterações sucessivas até março para consolidar decisões e artefatos.

\textbf{Fase 3 - Exemplos Aplicados (Fevereiro–Julho)}: Implementação prática do modelo em dois exemplos aplicados conduzidos entre fevereiro e julho.

\textbf{Fase 4 - Análise e Preparação (Agosto–Setembro)}: Análise dos dados coletados e preparação da defesa (documento final e apresentação).

A \textbf{Defesa de Tese} ocorrerá em \textbf{outubro do próximo ano}.
...
\textbf{Avaliação orientada a eixos.} A interpretação das evidências combina dados instrumentados e instrumentos docentes, organizada em quatro eixos: (i) \emph{Processo} — aderência a rituais ágeis e padrões de colaboração; (ii) \emph{Produto} — qualidade técnica e alinhamento ao TAPI; (iii) \emph{Conceitual} — domínio teórico evidenciado em provas/atividades e justificativas técnicas; (iv) \emph{Colaboração} — contribuição individual e dinâmica de equipe. Os eixos são ponderáveis e podem ser adaptados ao PPC.
...
O modelo conceitual proposto estabelece as fundações para a captura e integração de eventos em ambientes de Aprendizagem Baseada em Projetos (PBL). Ele se baseia em um conjunto de premissas e definições que garantem a coerência e a rastreabilidade dos dados coletados. A principal premissa é a adoção de práticas ágeis no desenvolvimento dos projetos, com o uso de um Termo de Abertura de Projeto Institucional (TAPI) bem estruturado, repositórios de código para controle de versão e sistemas de gestão de tarefas. O escopo do modelo se concentra nos processos pedagógicos e de desenvolvimento, não abrangendo processos administrativos externos ao módulo de ensino.

O framework reconhece a interação de múltiplos atores que desempenham papéis cruciais no ecossistema PBL. Os \textbf{Estudantes}, organizados em \textbf{Equipes}, são os protagonistas do processo, executando tarefas técnicas e gerando os dados primários que alimentam o gêmeo digital. As equipes operam com papéis definidos, como Product Owner e Scrum Master, e são responsáveis pelo planejamento e entrega de incrementos de software. O \textbf{Professor Orientador} atua como mediador do processo de aprendizagem, oferecendo acompanhamento pedagógico e feedback formativo contínuo. \textbf{Docentes de Eixo} contribuem com instrução especializada em áreas específicas, enquanto o \textbf{Parceiro Externo} enriquece a experiência ao apresentar um problema autêntico e validar as soluções propostas. A \textbf{Coordenação}, por sua vez, é responsável por garantir os recursos e a infraestrutura necessários, além de utilizar os indicadores agregados para a gestão acadêmica.

A dinâmica do ambiente PBL é capturada por meio de uma tipologia de eventos que abrange as diversas facetas do processo de aprendizagem. Os eventos \textbf{Acadêmicos} incluem interações formais de ensino, como instruções, registros de presença, atividades ponderadas e avaliações. Os eventos \textbf{Operacionais} refletem a aplicação de metodologias ágeis, como cerimônias de planejamento, reuniões diárias, revisões de sprint e retrospectivas. Os eventos \textbf{Técnicos}, extraídos de ferramentas de desenvolvimento, compreendem ações como commits, pull requests, issues, builds e testes. Por fim, os eventos \textbf{Documentais} registram a evolução dos artefatos do projeto, como o TAPI, relatórios de progresso e apresentações.

A rastreabilidade e a integração dos dados são asseguradas por um sistema de identificadores canônicos que reflete a hierarquia institucional. A cadeia de identificadores, que vai desde o nível macro das diretrizes curriculares até o nível micro dos artefatos de projeto, permite uma rastreabilidade completa. Essa estrutura garante a explicabilidade dos dados e a transparência do processo, desde as normativas institucionais até os produtos finais entregues pelas equipes.

Para garantir a sua eficácia e utilidade, o modelo foi projetado com base em um conjunto de requisitos de qualidade. A \textbf{Confiabilidade} é assegurada por meio de um processamento de dados determinístico e tolerante a falhas, capaz de lidar com dados faltantes ou fora de ordem. A \textbf{Latência} do sistema é projetada para atender às necessidades do ambiente educacional, com atualizações diárias e a possibilidade de consultas sob demanda. A \textbf{Escalabilidade} garante que o modelo possa ser aplicado a múltiplos módulos e turmas concorrentemente. A \textbf{Segurança} é tratada por meio de mecanismos de controle de acesso, trilhas de auditoria e criptografia dos dados. Por fim, a \textbf{Usabilidade} é garantida por meio de dashboards acessíveis e intuitivos, que oferecem visualizações com rastreabilidade completa dos dados.
...
A modelagem dinâmica do PBL, representada por meio de diagramas de sequência, ilustra as interações entre os diferentes atores e componentes do sistema ao longo do tempo. A seguir, são apresentados os principais fluxos de interação que caracterizam o processo de aprendizagem e desenvolvimento no contexto do PBL.
  ...
  \begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-01.png}
...
\end{figure}
  ...
  \begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-02.png}
...
\end{figure}
  ...
  \begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-07.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-14.png}
...
Esta subseção sintetiza a modelagem conceitual que estrutura o gêmeo digital orientado a eventos. Utilizamos dois modelos de classes complementares: (i) um núcleo de CEP (captura e organização de eventos educacionais) e (ii) o metamodelo de eventos no nível de módulo.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-01-ingestao-lote-validacao-schema.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-02-evento-presenca-pipeline.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-03-evento-entrega-artefato-rastreabilidade.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-04-deduplicacao-idempotencia.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-snapshot-app.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-03.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-04.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-05.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-06.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-08.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-09.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-10.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-11.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-12.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-13.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-15.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-05-eventos-atrasados-watermark.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-06-correlacao-join-por-chaves.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-07-validacao-versao-schema-migracao.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-08-erro-validacao-dlq-correcao.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-09-replay-reprocessamento-controlado.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-10-materializacao-sinais-para-docente.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-11-compl-pbl-01-aluno-inicia-atividade.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-12-compl-pbl-02-professor-corrige-lanca-nota.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-13-compl-pbl-03-revisao-de-nota.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-14-compl-pbl-07-planning-kanban.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-15-compl-pbl-08-branch-github.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-16-compl-pbl-11-presenca.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-17-compl-pbl-15-autoestudo.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-18-compl-pbl-18-pitch.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-19-compl-pbl-23-encerramento-sprint.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/seq-dt-20-compl-pbl-25-encerramento-modulo.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/SEQ-B22-coleta-periodica-de-eventos.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/SEQ-B23-deteccao-de-anomalia-de-atividade.png}
  \caption{Detecção de anomalias de atividade a partir de métricas de eventos.}
  \label{fig:dt-b23}
\end{figure}

Sinais de anomalia (queda abrupta/atividade atípica) disparam alertas e ações corretivas com o orientador.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-16.png}
  \caption{Diagrama de sequência PBL-16 — Feedback de autoestudo.}
  \label{fig:seq-pbl-16}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-17.png}
  \caption{Diagrama de sequência PBL-17 — Solicitação de extensão de prazo.}
  \label{fig:seq-pbl-17}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-18.png}
  \caption{Diagrama de sequência PBL-18 — Pitch com parceiro.}
  \label{fig:seq-pbl-18}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-19.png}
  \caption{Diagrama de sequência PBL-19 — Retrospectiva pós-pitch.}
  \label{fig:seq-pbl-19}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-20.png}
  \caption{Diagrama de sequência PBL-20 — Mudança no TAPI.}
  \label{fig:seq-pbl-20}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-21.png}
  \caption{Diagrama de sequência PBL-21 — Escalonamento de bloqueio.}
  \label{fig:seq-pbl-21}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-22.png}
  \caption{Diagrama de sequência PBL-22 — Mentoria da sprint.}
  \label{fig:seq-pbl-22}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-23.png}
  \caption{Diagrama de sequência PBL-23 — Encerramento de sprint.}
  \label{fig:seq-pbl-23}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-24.png}
  \caption{Diagrama de sequência PBL-24 — Preparação de demonstração final.}
  \label{fig:seq-pbl-24}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/seq/pbl-25.png}
  \caption{Diagrama de sequência PBL-25 — Encerramento de módulo.}
  \label{fig:seq-pbl-25}
\end{figure}
